...
export async function POST(req: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Remaining balance for your stay ${booking.checkIn.toISOString().split("T")[0]} to ${booking.checkOut.toISOString().split("T")[0]}`,
      ...
      cancel_url: `${baseUrl}/booking/pay-balance/${booking.id}?cancelled=true`,
    });
