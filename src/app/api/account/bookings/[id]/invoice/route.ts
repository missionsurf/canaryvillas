...
<div class="parties">
  <div class="party">
    <h3>From</h3>
    <p><strong>Canary Villas</strong>Corralejo, Fuerteventura<br/>Canary Islands, Spain<br/>info@canaryvillas.com</p>
  </div>
  <div class="party">
    <h3>To</h3>
    <p><strong>${booking.guestName}</strong>${booking.guestEmail}<br/>${booking.guestPhone ?? ""}</p>
  </div>
</div>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>${booking.villa.name}</strong><br/><span style="color:#64748b;font-size:13px;">${format(booking.checkIn, "d MMM yyyy")} → ${format(booking.checkOut, "d MMM yyyy")}</span></td>
    ...
    <tr>
      ...
      <td style="text-align:right;">€${booking.cleaningFee.toFixed(2)}</td>
      <td style="text-align:right;">€${booking.cleaningFee.toFixed(2)}</td>
    </tr>
    <tr class="total-row">
...
<p style="font-size:13px;color:#64748b;">Payment: ${booking.paymentMethod}${booking.stripePaymentId ? ` · Ref: ${booking.stripePaymentId}` : ""}</p>
<p style="font-size:13px;color:#64748b;margin-top:4px;">Guests: ${booking.guests} · Booking ref: ${ref}</p>

<div class="footer">
  <p>Thank you for choosing Canary Villas. We hope you have a wonderful stay!</p>
  <p>canaryvillas.com · info@canaryvillas.com · +44 7809 870561</p>
  <p style="margin-top:12px;" class="no-print"><button onclick="window.print()" style="background:#0284c7;color:#fff;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-size:13px;">Print / Save as PDF</button></p>
</div>
</body>
</html>`;
